...
export function webAppLd(
  opts: {
    ...
    features?: string[];
    /**
     * Only the pages that actually mount a client-side tool (the home converter, the recipe
     * scaler) require JavaScript — the programmatic ingredient/amount/fraction pages are static
     * content that merely link to the tool. Defaults to true for backward compatibility; those
     * static-page callers pass `interactive: false` (audit gramcup-audit-1, P3: "Static answer
     * pages declare WebApplication 'Requires JavaScript', although they contain no app").
     */
    interactive?: boolean;
    /** ISO date the underlying data/content was last verified or changed. */
    dateModified?: string;
  ...
  return {
    ...
    operatingSystem: 'Any',
    ...(opts.interactive === false ? {} : { browserRequirements: 'Requires JavaScript' }),
...
export function datasetLd(opts: {
  ...
  return {
    ...
    isBasedOn: opts.sources.map((s) => ({ '@type': 'CreativeWork', name: s.name, url: s.url })),
  };
}
